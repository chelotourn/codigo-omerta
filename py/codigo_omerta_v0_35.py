...
def _acusacion_falsa_y_descriptivas_falsas(c, s, sus, asig):
    """Verdad cuando hay al menos una acusación falsa (a cualquier sospechoso) Y
    todas las cartas descriptivas presentes también son falsas.
    Refleja: 'El que acusa no sabe. El que describe tampoco.'
    - Si no hay acusaciones en la ficha → falso (la afirmación no aplica).
    - Si hay acusación falsa pero no hay descriptivas → falso (segunda condición vacía)."""
    acusadores = [sid for sid, cid in asig.items()
                  if CATEGORIAS_CARTAS.get(cid) == "acusación"]
    if not acusadores:
        return False   # sin acusaciones: afirmación vacía → falso
    # Condición 1: al menos una acusación es falsa dado el culpable real
    hay_acusacion_falsa = any(
        not evaluar_carta_simple(asig[sid], c, sid, sus) for sid in acusadores
    )
    if not hay_acusacion_falsa:
        return False
    # Condición 2: todas las descriptivas presentes son falsas
    descriptivas = [sid for sid, cid in asig.items()
                    if CATEGORIAS_CARTAS.get(cid) == "descriptiva"]
    if not descriptivas:
        return False   # sin descriptivas la segunda condición es vacía → falso
    return all(not evaluar_carta_simple(asig[sid], c, sid, sus) for sid in descriptivas)
...
# 26: "El que acusa no sabe. El que describe tampoco. Busquen al culpable en otro lado."
# Verdad cuando hay al menos una acusación falsa (dirigida al declarante) Y
# todas las cartas descriptivas presentes también son falsas.
_CB[26] = _meta(lambda c, s, sus, asig: _acusacion_falsa_y_descriptivas_falsas(c, s, sus, asig))
    ...
    while len(fichas) < n_fichas and intentos < max_intentos:
        ...
        for sid in sosp_ids:
            pool = [
                ...
                # #26 "el que acusa no sabe, el que describe tampoco" - necesita que haya al menos
                # 1 carta de acusación Y 1 de descriptiva entre las cartas aún no usadas (slots restantes)
                and not (cid == 26 and not (
                    any(CATEGORIAS_CARTAS.get(c) == "acusación"   for c in ids_cartas if c not in cartas_usadas and c != 26) and
                    any(CATEGORIAS_CARTAS.get(c) == "descriptiva" for c in ids_cartas if c not in cartas_usadas and c != 26)
                ))
