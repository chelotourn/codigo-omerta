...
def validar_carta_64(asignacion: dict, culpable_id: int, sus: dict) -> bool:
    """Ya no se necesita validacion especial de inversion para la carta 64.
    Se mantiene la funcion por compatibilidad con generar_fichas; siempre devuelve True."""
    return True
...
CARTAS_INDIRECTAS = {
    ...
    # 65: "Si los acusados de la sala resultan ser inocentes, seguro que los que dudan hablan bajo amenaza y no podemos creerles."
    # A = al menos una carta de acusación en la ficha es falsa (hay acusado inocente)
    # B = todos los que tienen carta de duda mienten
    # Verdad jugable: A AND B  → alguna acusación falla Y todos los dudosos mienten
    # A falsa (todas las acusaciones son verdad) → carta falsa, B indeterminada
    65: _meta(lambda c, s, sus, asig:
        (lambda acusaciones:
            None if not acusaciones else (
                any(not evaluar_carta_simple(cid, c, sid, sus) for sid, cid in acusaciones)
                and all(not evaluar_carta_simple(cid, c, sid, sus)
                        for sid, cid in asig.items()
                        if CATEGORIAS_CARTAS.get(cid) == "duda")
            )
        )([(sid, cid) for sid, cid in asig.items() if CATEGORIAS_CARTAS.get(cid) == "acusación"])
    ),
    ...
    # A falsa (Archivista dice verdad) → carta falsa, B indeterminada
    # Archivista ausente → None (ficha inválida)
    ...
    # 72: "Si nadie se atreve a acusar directamente al culpable, entonces el dinero tiene algo que ver."
    # A = nadie acusa directamente al culpable (ninguna carta de acusación es verdad dado c)
    # B = el culpable es rico
    # Verdad jugable: A AND B  → ninguna acusación apunta al culpable Y el culpable es rico
...
def _antecedente_indirecta(carta_id: int, culpable_id: int, declarante_id: int,
    ...
    if carta_id == 65:   # A = alguna carta de acusación en la ficha es falsa
        acusaciones = [(sid, cid) for sid, cid in asig.items() if CATEGORIAS_CARTAS.get(cid) == "acusación"]
        if not acusaciones: return None   # sin acusaciones: A no aplica, ficha invalida
        return any(not evaluar_carta_simple(cid, c, sid, sus) for sid, cid in acusaciones)
...
TEXTOS_CARTAS = {
    ...
    22: "Desconfíen de todos los ricos. El porte da una confianza que cuesta distinguir de la inocencia.",
    ...
    72: "Si nadie se atreve a acusar directamente al culpable, entonces el dinero tiene algo que ver. El miedo rara vez es gratuito.",
}
    ...
    while len(fichas) < n_fichas and intentos < max_intentos:
        ...
        for sid in sosp_ids:
            pool = [
                ...
                # #65: necesita al menos 1 acusación y al menos 1 duda ya asignadas
                and not (cid == 65 and not (
                    any(CATEGORIAS_CARTAS.get(asignacion.get(x)) == "acusación" for x in asignacion) and
                    any(CATEGORIAS_CARTAS.get(asignacion.get(x)) == "duda"      for x in asignacion)
                ))
                # #72: necesita al menos 1 acusación ya asignada (para que A sea evaluable)
                #      y al menos 1 sospechoso rico en la partida (para que B sea posible)
                and not (cid == 72 and not any(CATEGORIAS_CARTAS.get(asignacion.get(x)) == "acusación" for x in asignacion))
                and not (cid == 72 and not any(SOSPECHOSOS[x]["clase"] == "rico" for x in sosp_ids))
