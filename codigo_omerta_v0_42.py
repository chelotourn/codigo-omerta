...
TEXTOS_CARTAS = {
    ...
    7:  "Solo el El Vagabundo o el Crupier estaban en la escena del crimen. Uno de ellos lo sabe.",
    ...
    # INDIRECTAS
    65: "Escucho a quienes defienden y quienes describen al culpable, pero si unos dicen la verdad los otros mienten por miedo o por vergüenza.",
    ...
    68: "Si el Crupier miente, quien lo hizo no era joven. Son los únicos a los que él nunca defendería.",
...
class Ficha:
    ...
    distrito: int = 1            # 1 = Distrito Industrial | 2 = Distrito Comercial | 3 = Distrito Síntesis (ficha-conclusión)
...
#            (id=3, "Distrito Síntesis"), construido dinámicamente. A partir
...
def generar_caso(n_fichas: int, modo: str, cantidad_fija: Optional[int],
                  dificultad: str, max_intentos_ficha: int = 200_000,
...
def main():
    ...
    print("  [5] Caso      (N fichas + ficha-conclusión, Distrito Síntesis)")
    nivel = pedir_entero("  Elegí una opción [1-5]: ", minimo=1, maximo=5)

    # ── Modo Caso: N fichas + intento de ficha-conclusión ──
    if nivel == 5:
        print("\n  ¿Dificultad del Caso?")
        print("  [1] Urbano    (mínimo 3 sospechosos)")
        print("  [2] Metrópoli (mínimo 4 sospechosos)")
        print("  [3] Omertà    (mínimo 5 sospechosos)")
        sub_nivel = pedir_entero("  Elegí una opción [1-3]: ", minimo=1, maximo=3)
        dificultad_caso = {1: "urbano", 2: "metropoli", 3: "omerta"}[sub_nivel]
        minimo_dif, _max_dif = RANGO_SOSPECHOSOS_POR_DIFICULTAD[dificultad_caso]

        n_fichas_caso = pedir_entero(
            f"\n  ¿Cuántas fichas tiene el Caso?\n"
            f"  (Si es menor a {minimo_dif} no se intentará la ficha-conclusión "
            f"para {dificultad_caso.upper()})\n"
            f"  Ingresá [3-50]: ",
            minimo=3, maximo=50
        )
        if n_fichas_caso < minimo_dif:
            print(f"  → {n_fichas_caso} < mínimo ({minimo_dif}): se generarán las fichas "
                  f"SIN intentar el cierre del Caso.")
        else:
            print(f"  → Se generarán {n_fichas_caso} fichas e intentará el cierre del Caso.")

        seed_caso = pedir_seed()
        jugable_caso = pedir_entero(
            "\n  ¿Ofuscar Respuestas?\n  [0] No, [1] Sí: ",
            minimo=0, maximo=1
        ) == 1
        ...
        resultado_caso = generar_caso(
            n_fichas=n_fichas_caso,
            modo=modo,
            cantidad_fija=None,
            dificultad=dificultad_caso,
            seed=seed_caso,
        )
        ts_archivo = datetime.now().strftime("%Y%m%d_%H%M%S")
        ruta_base = os.path.join(".", f"caso_{dificultad_caso}_{ts_archivo}")
        exportar_caso(resultado_caso, ruta_base, jugable=jugable_caso)

        print()
        print("╔══════════════════════════════════════════════╗")
        if resultado_caso["ficha_conclusion"] is not None:
            print(f"║  Listo. Caso cerrado con ficha-conclusión.    ║")
        elif resultado_caso["descartado"]:
            print(f"║  Caso agotó reintentos sin cerrar.            ║")
        else:
            print(f"║  Caso generado sin ficha-conclusión.          ║")
        print("╚══════════════════════════════════════════════╝")
        return
